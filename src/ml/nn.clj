...
(defn back-prop [network network-activations target]
...
(defn train [network learning-rate [item target]]
  (let [activations (forward-prop network item)
        errors (back-prop network activations target)]
...
(defn batched-train [network learning-rate batch]
  (let [chunk-size (quot (count batch) n-cpu)
        ...
        batch-errors (doall (chunked-pmap #(back-prop network %1 (second %2)) chunk-size batch-activations batch))]
